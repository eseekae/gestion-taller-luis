...
import { 
  ...
  Rocket, Clock, Tag, Minus, Banknote, Edit3, ChevronDown
} from 'lucide-react'
...
export default function RegistroPedido() {
  ...
  const agregarAlCarrito = () => {
    ...
    if (tallaSeleccionada === 'ESPECIAL') {
      if (!precioManualEspecial) return alert("Ingresa el precio")
  ...
  const guardar = async (e: any) => {
    ...
    if (telefono.length !== 8) return alert("Teléfono debe tener 8 números")
    setLoading(true)
    try {
      ...
      const { data: ped } = await supabase.from('pedidos').insert([{
        cliente_id: cli.id, total_final: totalConDescuento, colegio: colegio || 'Particular',
      ...
      if (Number(montoPagado) > 0) {
        await supabase.from('pagos').insert([{ pedido_id: ped.id, monto: Number(montoPagado), metodo_pago: metodoPago }])
      }
  ...
  const inputStyle = { 
    ...
    appearance: 'none' as const, // ESTO MATA EL ESTILO DEL SISTEMA
    WebkitAppearance: 'none' as const,
    boxSizing: 'border-box' as const,
    outline: 'none'
  }

  const cardStyle = { backgroundColor: '#fff', padding: '20px', borderRadius: '28px', border: '4px solid #000', boxShadow: '8px 8px 0px #000', marginBottom: '25px' }
  ...
  return (
    <main style={{ minHeight: '100vh', backgroundColor: '#f8fafc', padding: '30px 15px' }}>
      ...
      {/* 💣 CSS GLOBAL QUE FUERZA EL NEGRO SÍ O SÍ */}
      <style jsx global>{`
        ...
        <form onSubmit={guardar}>
          ...
          <div style={cardStyle}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginTop: '10px' }}>
              <button type="button" onClick={() => setTipoEntrega('agendada')} style={{ padding: '14px', borderRadius: '16px', border: '4px solid #000', fontWeight: '900', backgroundColor: tipoEntrega === 'agendada' ? '#fbbf24' : '#fff' }}>AGENDAR</button>
              <button type="button" onClick={() => setTipoEntrega('inmediata')} style={{ padding: '14px', borderRadius: '16px', border: '4px solid #000', fontWeight: '900', backgroundColor: tipoEntrega === 'inmediata' ? '#4ade80' : '#fff' }}>INMEDIATA</button>
            </div>
          ...
          <div style={cardStyle}>
            ...
            <div style={{ display: 'grid', gap: '15px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <div style={{ padding: '16px 12px', border: '4px solid #000', borderRadius: '16px', fontWeight: '950', backgroundColor: '#e2e8f0' }}>+569</div>
          ...
          {/* PRENDAS - AQUÍ ESTÁ EL STOCK NEGRO */}
          <div style={cardStyle}>
            ...
            <div style={{ ...cardStyle, borderStyle: 'dashed' }}>
              {carrito.map((item) => (
                <div key={item.tempId} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '2px solid #f1f5f9' }}>
                  ...
                  <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}><span style={{ fontWeight: '950' }}>${(item.precio * item.cantidad).toLocaleString()}</span><button onClick={() => setCarrito(carrito.filter(c => c.tempId !== item.tempId))} style={{ color: '#ef4444', border: 'none', background: 'none' }}><X size={20}/></button></div>
                </div>
              ))}
          ...
          <textarea style={{ ...inputStyle, marginTop: '25px', height: '100px' }} placeholder="NOTAS / OBSERVACIONES" value={observaciones} onChange={e => setObservaciones(e.target.value)} />
        </form>
      </div>
    </main>
  )
